...
def save_stock_prices(start_date, end_date):
    ...
    while current_date <= end_date:
        if request_count >= 6:
            ...
            if elapsed_time < 3600:  # 一時間未満の場合は待機
        ...
        if response.status_code == 404:
            ...
            with open(f"raw_data/japan-all-stock-prices/{date_str}.csv", "wb") as f:
                ...
                print(f"Failed to download data for {date_str}, error code: {response.status_code}")  # ここを response.status から response.status_code に変更
